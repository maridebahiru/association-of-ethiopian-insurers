          ...
          <div key={colIndex} className="flex-1 flex flex-col" style={{ gap: `${gap}px` }}>
            {col.map((img) => (
              <motion.div
                ...
                className="relative cursor-pointer overflow-hidden rounded-2xl bg-white border border-slate-200 shadow-sm group"
              >
                <img 
                  src={img.src} 
      ...
      <AnimatePresence>
        {selectedImage && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/90 backdrop-blur-sm p-4 sm:p-10 cursor-zoom-out"
          ...
          >
            ...
            <motion.div 
              ...
              className="absolute bottom-10 left-1/2 -translate-x-1/2 text-white bg-slate-800/80 px-6 py-3 rounded-full backdrop-blur-md border border-slate-700 shadow-xl"
            >
