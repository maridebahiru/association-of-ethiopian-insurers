...
export default function Features() {
  ...
  return (
    <section className="py-32 bg-[#050505] relative z-10 border-t border-white/5" id="resources">
      <div className="max-w-7xl mx-auto px-6">
        <div className="mb-20 text-center flex flex-col items-center">
          ...
          >
            Insights & <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-500 to-yellow-400">Resources</span>
          </motion.h2>
          <motion.p 
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            ...
            className="text-white/50 max-w-2xl text-xl font-light"
          >
          ...
          {cards.map((card, i) => (
            <Link to={card.link} key={i}>
              <motion.div
                ...
                className="h-full p-8 pb-10 rounded-[2.5rem] bg-gradient-to-b from-white/[0.04] to-transparent border border-white/[0.08] hover:border-amber-500/30 hover:bg-white/[0.06] transition-all group flex flex-col items-start"
              >
                <div className="w-16 h-16 rounded-[1.5rem] bg-white/5 flex items-center justify-center mb-8 shadow-inner border border-white/10 group-hover:scale-110 group-hover:rotate-3 transition-all">
                  {card.icon}
                </div>
